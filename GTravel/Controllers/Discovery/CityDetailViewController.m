...
- (void)onMapBarButtonItem:(UIBarButtonItem *)item
{
    ...
    mapViewController.cityItem = self.cityItem;
...
- (void)viewDidLoad
{
    ...
    self.strURL = [self.cityItem.detailURL stringByAppendingFormat:@"?userid=%@", self.model.userItem.userID];
...
{
    if ([request.URL.absoluteString rangeOfString:self.cityItem.detailURL].location == NSNotFound) {
...
- (void)onShareButton:(UIButton *)button
{
    ...
    self.shareBottomView.hidden = NO;
    [UIView animateWithDuration:0.5
                     animations:^{

                       self.shareBottomView.alpha = 0.7;
